import { AiFacilitatorChatRequest, AiPromptBoardRequest, AiStepAssistRequest, AiStepAssistResponse, AiTextResponse } from '@/lib/contracts/api';
import { ToolFieldValue } from '@/types';

const wait = (timeMs: number) => new Promise((resolve) => globalThis.setTimeout(resolve, timeMs));

const formatProjectContext = (request: { project: AiPromptBoardRequest['project'] | AiFacilitatorChatRequest['project'] | AiStepAssistRequest['project'] }) => {
    const activeContext = [
...
function buildPromptBoardReply(request: AiPromptBoardRequest): string {
    return [
...
function buildChatReply(request: AiFacilitatorChatRequest): string {
...
function buildStepAssistResponse(request: AiStepAssistRequest): AiStepAssistResponse {
...
export const aiGateway: AiGateway = new LocalMvpAiGateway();
